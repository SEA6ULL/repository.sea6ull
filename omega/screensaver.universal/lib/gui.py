...
# Track whether the per-session startup cache rebuild has already been done.
# Module-level so it persists across Screensaver instances in the same session.
_STARTUP_CACHE_DONE = False
...
class Screensaver(xbmcgui.WindowXMLDialog):
    ...
    def _get_settings(self):
        ...
        self.cache_size = ADDON.getSettingInt('cache_size')
        self.slideshow_show_clearlogo = ADDON.getSettingBool('show_clearlogo')
        ...
        # clearlogo overlay for plugin type (control 200)
        try:
            self.clearlogo_img = self.getControl(200)
            self.clearlogo_img.setVisible(False)
        except Exception:
            self.clearlogo_img = None
    ...
    def _start_show(self, items):
        ...
        thread = img_update(data=self._get_items,
                            slideshow_type=self.slideshow_type,
        ...
        while (not self.Monitor.abortRequested()) and (not self.stop):
            ...
            for img in items[self.offset:]:
                ...
                if not self.startup:
                    ...
                    self.startup = False
                # Set clearlogo image and visibility in sync with fanart
                # Works for type 0 (video fanart) and type 3 (plugin) - both use [fanart, logo, label] triples
                if self.slideshow_type in (0, 3) and self.clearlogo_img is not None and self.slideshow_show_clearlogo:
                    _logo_path = img[1] if len(img) > 2 else ''
                    # For library fanart (type 0), logo is an image:// URL - check it's non-empty
                    # For plugin (type 3), logo is a local file path - check it exists
                    if _logo_path and self.slideshow_type == 3 and not os.path.exists(_logo_path):
                        _logo_path = ''
                    if _logo_path:
                        self.clearlogo_img.setImage(_logo_path, False)
                        self.clearlogo_img.setVisible(True)
                    else:
                        self.clearlogo_img.setVisible(False)
                else:
                    if self.clearlogo_img is not None:
                        self.clearlogo_img.setVisible(False)
                ...
                if self.slideshow_name != 0:
                    # For plugin type 3 and video fanart type 0, label is img[2]; music type 1 still img[1]
                    _label = img[2] if (self.slideshow_type in (0, 3) and len(img) > 2) else img[1]
                    if self.slideshow_name == 1: # filename
                        if self.slideshow_type == 2:
                            if self.slideshow_path.startswith('plugin://'):
                                NAME, EXT = os.path.splitext(os.path.basename(_label))
                            else:
    ...
    def _get_items(self, update=False):
        ...
        # ---- type 3: video plugin with local cache ----
        if self.slideshow_type == 3:
            plugin_path = load_plugin_path()
            log('plugin path: %s' % plugin_path)
            if plugin_path and plugin_path.startswith('plugin://'):
                self.items = read_plugin_cache_pairs()
                log('plugin cache pairs: %d' % len(self.items))
            else:
                self.items = []
            # fall back to video fanart if cache is empty
            if not self.items:
                log('plugin cache empty – falling back to video fanart')
        ...
        # query the db
        if not self.slideshow_type == 2 and not self.slideshow_type == 3:
            self.items = []
                ...
                if 'result' in json_response and json_response['result'] != None and method[1] in json_response['result']:
                    for item in json_response['result'][method[1]]:
                        if 'fanart' in item['art']:
                            fanart_url = item['art']['fanart']
                            # For video fanart (type 0), also grab clearlogo from art dict
                            # Kodi's image:// URLs work directly with setImage for library art
                            logo_url = ''
                            if self.slideshow_type == 0:
                                logo_url = (item['art'].get('clearlogo') or
                                            item['art'].get('clearart') or '')
                            self.items.append([fanart_url, logo_url, item['label']])
        # randomize
    ...
    def _exit(self):
        ...
        self.stop = True
        # hide clearlogo overlay on exit
        try:
            if self.clearlogo_img:
                self.clearlogo_img.setVisible(False)
        except Exception:
            pass
        self._clear_prop('ClearLogo')
...
class img_update(threading.Thread):
    def __init__( self, *args, **kwargs ):
        ...
        self._slideshow_type = kwargs.get('slideshow_type', 0)
        self._plugin_path = kwargs.get('plugin_path', '')
        self._cache_size = kwargs.get('cache_size', 50)
    ...
    def run(self):
        global _STARTUP_CACHE_DONE
        # For plugin type: on the very first screensaver activation after Kodi
        # starts, rebuild the cache so images stay current.
        # If _get_items already built a fresh cache (empty cache with path set),
        # skip the rebuild to avoid duplicating work.
        if self._slideshow_type == 3 and self._plugin_path:
            if not _STARTUP_CACHE_DONE:
                _STARTUP_CACHE_DONE = True
                # Only do the startup rebuild if the cache already had content
                # (meaning it existed from a previous session and may be stale).
                # If cache was just built by _get_items, skip this.
                if plugin_cache_exists():
                    log('img_update: startup plugin cache refresh (60s delay)')
                    ...
                    log('img_update: cache freshly built, skipping startup rebuild')
            # hourly refresh loop for plugin type
            while (not self.Monitor.abortRequested()) and (not self.stop):
                count = 0
                while count != 3600:
                    xbmc.sleep(1000)
                    ...
                    if self.Monitor.abortRequested() or self.stop:
                        return
                log('img_update: hourly plugin cache rebuild')
                build_plugin_cache(self._plugin_path, self._cache_size, startup_delay=False)
                self._get_items(True)
            return
        # Original behaviour for all other types
        while (not self.Monitor.abortRequested()) and (not self.stop):
